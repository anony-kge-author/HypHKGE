...
parser.add_argument(
    "--model", default="RotE1", choices=all_models, help="Knowledge Graph embedding model"
)
parser.add_argument(
...
def train(args):
    ...
    #result for each rel in WN18RR
    if args.dataset=="WN18RR":

        logging.info("--------------------------------------------")
        # 依据关系对测试集进行划分;测试集中共包含11种关系，标号为0-10
        rel_name = ["_also_see", "_derivationally_related_form", "_verb_group", "_has_part", "_hypernym",
                    "_instance_hypernym", "_member_meronym", "_member_of_domain_region", "_member_of_domain_usage",
                    "_similar_to", "_synset_domain_topic_of"]
        for i in range(11):
            idx_rel = test_examples[:, 1] == i
            sub_test_example = test_examples[idx_rel, :]
            test_metrics = avg_both(*model.compute_metrics(sub_test_example, filters))
            logging.info("关系:"+rel_name[i])
            logging.info("该关系Test三元组数量：")
            logging.info(sub_test_example.shape[0])
            logging.info(format_metrics(test_metrics, split="test"))
            logging.info("--------------------------------------------")
