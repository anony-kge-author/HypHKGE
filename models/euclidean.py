...
EUC_MODELS = ["TransE", "CP", "MurE", "RotE", "RefE", "AttE", "RotE1"]
...
class RotE1(BaseE):
    ...
    def __init__(self, args):
        super(RotE1, self).__init__(args)
        self.rel_diag = nn.Embedding(self.sizes[1], self.rank)
        self.rel_diag.weight.data = 2 * torch.rand((self.sizes[1], self.rank), dtype=self.data_type) - 1.0
        self.sim = "dist"
        self.w = nn.Embedding(self.sizes[1], int(self.rank/2))
        self.w.weight.data = self.init_size * torch.randn((self.sizes[1], int(self.rank/2)), dtype=self.data_type)

    def get_queries(self, queries: torch.Tensor):
        """Compute embedding and biases of queries."""
        w = self.w(queries[:, 1]).reshape(-1,1)
        w = w.repeat(1,2).reshape(-1,self.rank)

        lhs_e = givens_rotations(self.rel_diag(queries[:, 1]), self.entity(queries[:, 0]))*w + self.rel(queries[:, 1])
